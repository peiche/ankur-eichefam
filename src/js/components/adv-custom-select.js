...
(function () {
  var AdvSelect = function (element) {
    ...
    this.trigger = this.element.getElementsByClassName(
      "js-adv-select__control",
    )[0];
    this.triggerLabel = this.trigger.getElementsByClassName(
      "js-adv-select__value",
    )[0];
  ...
  function createAdvStructure(select) {
    ...
    if (optgroup) {
      ...
      optgroupLabel = document.getElementById(
        optgroupClone.getAttribute("describedby"),
      );
    }
    ...
    if (select.optGroups.length > 0) {
      ...
      for (var i = 0; i < select.options.length; i++) {
        dropdownCode =
          dropdownCode + getOptionCode(select, select.options[i], optionClone);
      }
    }
  ...
  function getOptionCode(select, option, optionClone) {
    ...
    for (var i = 0; i < select.optionData.length; i++) {
      optionHtml = optionHtml.replace(
        "{" + select.optionData[i] + "}",
        option.getAttribute("data-" + select.optionData[i]),
      );
    }
    return optionHtml;
  }
  ...
  function updateTriggerLabel(select) {
    ...
    select.triggerLabel.innerHTML = select.dropdown.querySelector(
      '[aria-selected="true"]',
    ).innerHTML;
  }
  ...
  function updateNativeSelect(select, selectedValue) {
    var selectedOption = select.select.querySelector(
      '[value="' + selectedValue + '"',
    );
    select.select.selectedIndex = Util.getIndexInArray(
      select.options,
      selectedOption,
    );
  ...
  function keyboardCustomSelect(select, direction) {
    ...
    var dropdownOption = select.dropdown.querySelector(
      '[data-value="' + select.options[index].getAttribute("value") + '"]',
    );
