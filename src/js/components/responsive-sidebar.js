...
(function () {
  ...
  function getPreventScrollEl(element) {
    var scrollEl = false;
    var querySelector = element.element.getAttribute(
      "data-sidebar-prevent-scroll",
    );
  ...
  function showSidebar(sidebar) {
    ...
    if (sidebar.preventScrollEl)
      sidebar.preventScrollEl.style.overflow = "hidden";
  }
  ...
  function initSidebarResize(sidebar) {
    ...
    var mainContent = document.getElementsByClassName(
      sidebar.contentReadyClass,
    );
    if (mainContent.length > 0)
      Util.removeClass(mainContent[0], sidebar.contentReadyClass);
